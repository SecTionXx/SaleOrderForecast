/**
 * categoryAnalysis.js - Sales Category Analysis API Endpoint
 * Provides server-side analysis capabilities using the data processing service
 */
...
module.exports = async (req, res) => {
  console.log("Backend: /api/categoryAnalysis invoked...");
  ...
  try {
    ...
    const hasPermission = await authorize(user, 'analytics:read');
    if (!hasPermission) {
      res.status(403).json({
        success: false,
        error: 'Access Denied',
  ...
  try {
    ...
    if (global.lastAnalysisTime && (currentTime - global.lastAnalysisTime) < (60000 / requestsPerMinute)) {
      console.warn("Rate limit exceeded for category analysis API");
      res.status(429).json({
        success: false,
    ...
    global.lastAnalysisTime = currentTime;

    // --- Process the category analysis request ---
    try {
      console.log(`Backend: Processing category analysis request with ${salesData.length} data points`);
      
      // Analyze sales data by category using server-side business logic
      const analysisResults = dataProcessingService.analyzeSalesByCategory(salesData);
      ...
      console.error("Backend: Error processing category analysis:", processingError);
      
      res.status(422).json({
        success: false,
        error: 'Analysis Processing Error',
        code: 'PROCESSING_ERROR',
        message: processingError.message || 'Error analyzing sales data',
        timestamp: new Date().toISOString()
      });
    }
  } catch (error) {
    // --- Handle Unexpected Errors ---
    console.error("Backend: Unexpected error in category analysis API:", error);
